...
class Site:
 ...
 def page(self,r,title,body):
  ...
  if r.user:auth=f'<a href="/area">I miei lavori</a><form method="post" action="/logout" class="inline">{r.csrf()}<button class="text-button">Esci</button></form>'
  ...
  if r.method=='POST':
   ...
   try:
    ...
    if r.path=='/registrati':
     ...
     body=f'''<section class="narrow panel code-success"><span class="eyebrow">IL TUO ACCOUNT PERSONALE</span><h1>Conserva le credenziali.</h1><p>Puoi entrare nei tuoi lavori con il codice di accesso oppure con lo username scelto.</p><div class="practice-code" aria-label="Codice di accesso">{esc(code)}</div>{username_note}<p class="notice">Ti abbiamo inviato anche una copia del codice via email. La stessa email riceverà gli avvisi di consegna e revisione.</p><a class="button full" href="/nuovo">Richiedi il tuo primo lavoro ↗</a></section>'''
