...
class Site:
 ...
 def route(self,r):
  ...
  if p=='/':return self.page(r,'La tua tesi comincia a prendere forma',public_landing(self.public_reviews())),200,[]
 ...
 def dashboard(self,r):
  ...
  actions='<a class="button" href="/gestione/recensioni">Gestisci recensioni ↗</a>' if admin else '<a class="button" href="/nuovo">Nuova richiesta +</a>'
  body=f'<section class="workspace"><div class="page-heading"><div><span class="eyebrow">{"Pannello di gestione" if admin else "Area personale"}</span><h1>Ciao, {esc(r.user["name"])}.</h1><p>{"Le richieste da seguire, tutte qui." if admin else "Qui la tua tesi prende forma, un passo alla volta."}</p></div>{actions}</div>{mail}<div class="stats">'+''.join(f'<div><strong>{counts[k]:02}</strong><span>{v}</span></div>' for k,v in STATUS.items())+f'</div><div class="toolbar"><div class="filters"><a class="filter {"selected" if not status else ""}" href="/area">Tutti</a>{filters}</div><form method="get" class="search"><label class="sr-only" for="search">Cerca un lavoro</label><input id="search" name="q" placeholder="Cerca un lavoro…" value="{esc(search)}"><button aria-label="Cerca">⌕</button></form></div><div class="project-list">{cards}</div></section>'
...
def public_landing(reviews):
 ...
 if not cards:cards=f'<div class="reviews-empty"><span class="fb-review-mark">f</span><div><h3>Le parole di chi ci ha scelto.</h3><p>Leggi le recensioni direttamente sulla pagina Facebook di FormaTesi.</p></div><a class="button secondary" href="{FB_REVIEWS}" target="_blank" rel="noopener">Leggi su Facebook ↗</a></div>'
 ...
 base=base.replace('Seleziona l’università nella richiesta: il progetto sarà organizzato in base alle informazioni che ci fornisci.','eCampus, Pegaso, Mercatorum e San Raffaele sono solo alcune delle università seguite. Se il tuo ateneo non è nell’elenco, puoi inserirlo liberamente.')
